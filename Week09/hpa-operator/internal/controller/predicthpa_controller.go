...
import (
	...
	hpav1 "github.com/lyzhang1999/hpa-operator/api/v1"
...
func (r *PredictHPAReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, types.NamespacedName{Name: predictHPA.Spec.DeploymentName, Namespace: req.Namespace}, deployment); err != nil {
...
func getRecommendedReplicas(predictHost string) (int32, error) {
	...
	fmt.Println(fmt.Sprintf("http://%s/predict", predictHost))
	if err != nil {
		return 0, err
	}
